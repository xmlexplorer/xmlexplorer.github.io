...
const AD_SLOT = '3038002503';
// Loaded without the `?client=` query param, matching about.html. Note that
// this is *not* what keeps page-level Auto ads off: Google runs Auto ads
// through any AdSense ad unit code, so the invasive formats (anchors,
// vignettes, side rails) that would inject outside this component have to stay
// disabled per-site in the AdSense console.
 ...
 * window it happily picks a few hundred pixels and eats the bottom of the app.
 * With explicit width/height and no data-ad-format, AdSense serves the size we
 * asked for.
 */
const BANNER_SIZES = [
...
export const DONATE_URL = 'https://www.paypal.com/donate/?hosted_button_id=U6HSRT57R85C8';

export const GITHUB_URL = 'https://github.com/xmlexplorer/xmlexplorer.github.io';

/**
 * Bottom-of-app status bar: donate/source links, plus an AdSense banner on web.
 *
  ...
  return (
    ...
    >
      <span style={{ flexShrink: 0 }}>
        <Space>
          <Button type="link" size="small" icon={<HeartOutlined />} onClick={() => openExternal(DONATE_URL)}>
            {t('donate')}
          </Button>
          <Button type="link" size="small" icon={<GithubOutlined />} onClick={() => openExternal(GITHUB_URL)}>
            {t('actions.code')}
          </Button>
        </Space>
      </span>
